...
export async function GET(_: NextRequest, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data: config } = await db.from("admin_config").select("bidding_timer_seconds,scoreboard_timer_seconds,card_confirmation").eq("singleton", true).single();
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      const next = applyAction(previous, participant.id, { type: "heartbeat", actionId: `presence-${participant.id}-${Math.floor(now.getTime() / 5000)}` }, config?.bidding_timer_seconds ?? 20, now, config?.scoreboard_timer_seconds ?? 30);
