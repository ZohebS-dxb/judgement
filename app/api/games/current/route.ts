...
export async function GET() {
  try {
    const db = adminDb();
    const { data: game } = await db.from("games").select("id,status,mode,custom_rounds,created_by_player_id,created_at,player_count").in("status", ["lobby","in_progress"]).maybeSingle();
